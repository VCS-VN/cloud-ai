...
- When modifying generated project detail routes/components, use optional chaining and nullish fallbacks for every data/entity read at every nested level, including values from queries, params, providers, API payloads, and nested product/store/order/customer fields before rendering or computing values. Examples: `store?.name?.trim()`, `storeDetail?.setting?.currency ?? 'AUD'`, `product?.descriptions?.trim() ?? ''`, `product?.category?.name`, `product?.images?.[0]`, `product?.models?.[0]?.name`, `product?.models?.length ?? 0`, `(order?.items ?? []).map(...)`. Treat optional chaining as the default style even when TypeScript currently marks a field as required. Only use direct property access after an explicit guard has proven the parent exists in that branch; nullable children still use `?.`/`??`.
