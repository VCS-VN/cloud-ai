...
2. REWRITE DESIGN.md by overwriting it (`cat > DESIGN.md <<'EOF'` … `EOF` with the complete new file): a new visual identity (palette as 15 hex color tokens in the YAML front-matter, typography, components, layout) driven by the skill and the request above. Honor any user-specified token values listed above. This file is the source of truth.
...
5. Update the storefront UI so it matches DESIGN.md (palette, typography, spacing, radii, components, layout): tailwind.config.ts, src/styles/* (if present), and the storefront components/routes. For each file, `cat <path>` to read it, then `cat > <path> <<'EOF'` … `EOF` with the complete updated contents (single `>`, never `>>`).
