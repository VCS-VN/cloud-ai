...
- Cart item id is always the selected model id. Adding the same model id combines quantity. Updating an existing item SETS quantity (not "add more"). Removal is an explicit `removeItem(id)` action (user mode calls `DELETE /api/v1/carts/${id}`) — do NOT remove by calling `updateItemQuantity(id, 0)`.
