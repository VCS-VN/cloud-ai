...
- The design-taste-frontend skill is preloaded inline in the system prompt (see the <design_taste_skill> block). AUTHOR DESIGN.md first (full 8 sections + YAML palette) via apply_patch; refer back to the inlined taste-skill content for visual decisions. There is no project_read_design_rules tool — use shell with cat to read DESIGN.md after it exists.
