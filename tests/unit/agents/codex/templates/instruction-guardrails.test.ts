...
describe("codex storefront instruction guardrails", () => {
  it("requires opaque token-backed surfaces for shadcn overlays and suggestions", async () => {
    ...
    expect(componentSpec).toContain("page/header backgrounds must never show through suggestion rows");
    expect(initSystem).toContain("SHADCN PRIMITIVE STYLING RULES");
    expect(editSystem).toContain("Shadcn UI primitives in `src/components/ui/*` are runtime-owned");
  });

  it("forbids product-detail render-loop state sync from query data", async () => {
    const [uiRules, productDetailSpec, initSystem, editSystem] = await Promise.all([
      readTemplate("templates/project-rules/ui-design.md"),
    ...
    expect(productDetailSpec).toContain("Track `selectedModelId` with `useState<string | null>(null)`");
    expect(productDetailSpec).toContain("Do not reset selectedImageIndex in an effect");
  ...
  it("requires optional chaining and nullish fallbacks for every generated data/entity read", async () => {
    const [dataRules, productDetailSpec, initSystem, editSystem, storeRuntime, codeAgent] = await Promise.all([
    ...
    for (const body of [dataRules, initSystem, editSystem, storeRuntime, codeAgent]) {
    ...
    expect(productDetailSpec).toContain("never call `.map` or `.length` on product.models directly");
    expect(initSystem).toContain("SAFE DATA ACCESS RULES");
    expect(initSystem).toContain("Treat optional chaining as the default style");
    expect(initSystem).toContain("(order?.items ?? []).map(...)");
    expect(editSystem).toContain("Product detail updates must return loading/error/missing-product UI");
    expect(storeRuntime).toContain("Treat optional chaining as the default style");
  ...
  it("forbids Tailwind v3 @apply with group or peer marker utilities", async () => {
    const [uiRules, initSystem, editSystem, builderRunSource] = await Promise.all([
      readTemplate("templates/project-rules/ui-design.md"),
      readTemplate("templates/codex-builder/init/system.md"),
      readTemplate("templates/codex-builder/foundation/edit-system.md"),
    ...
    expect(initSystem).toContain("@apply should not be used with the 'group' utility");
    expect(editSystem).toContain("@apply should not be used with the 'group' utility");
    expect(builderRunSource).toContain("tailwind_v3_apply_rules");
    expect(builderRunSource).toContain("@apply should not be used with the 'group' utility");
  });
  ...
  it("requires explicit product model selection, cart payload, toasts, quantity sync, and read-more behavior", async () => {
    ...
    for (const body of [productDetailSpec, storeRuntime]) {
      ...
      expect(body).toContain("toast.error('Please choose a product option first')");
      expect(body).toContain("const payload = { product, model: selectedModel, quantity }");
      expect(body).toContain("addItem(payload)");
      expect(body).toContain("toast.success");
      expect(body).toContain("Read more / Read less");
      expect(body).not.toContain("Add/update button is disabled until selectedModel.id exists");
      expect(body).not.toContain("confirm button is disabled until selectedModel.id exists");
    }

    expect(productDetailSpec).toContain("Do NOT disable the primary cart button solely because selectedModel is missing");
    expect(productDetailSpec).toContain("If the selected model already exists in cart, the quantity input MUST show that cart quantity");
    expect(productDetailSpec).toContain("Confirm button inside sheet follows the same selected-model validation");
  ...
  it("keeps legacy store prompt guidance aligned with overlay and render-loop guardrails", () => {
    ...
    expect(prompt).toContain("useState<string | null>(null) for selectedModelId");
    ...
    expect(prompt).toContain("missing-product UI before reading product fields");
    expect(prompt).toContain("initial selectedModelId is null");
    expect(prompt).toContain("toast.error('Please choose a product option first')");
    expect(prompt).toContain("const payload = { product, model: selectedModel, quantity }");
    expect(prompt).toContain("toast.success");
    expect(prompt).not.toContain("rounded-2xl bg-white p-3 shadow-lg shadow-black/5");
    expect(prompt).not.toContain("useState<ProductModel | undefined>(undefined) with initialModel");
  });

  it("generates preview-safe Vite config, dev command, and checkout placeholders", async () => {
    const [packageTemplate, viteConfigTemplate, checkoutSpec] = await Promise.all([
    ...
    expect(viteConfigTemplate).toContain("process.env.VITE_PREVIEW_PUBLIC_HOST");
    expect(viteConfigTemplate).toContain("`${previewProjectId}-preview.${previewDomain}`");
    ...
    expect(checkoutSpec).toContain("Do NOT seed form defaultValues with fake customer data");
