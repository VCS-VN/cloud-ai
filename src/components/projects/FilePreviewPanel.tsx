...
export function FilePreviewPanel({ node }: FilePreviewPanelProps) {
  ...
  return (
    <section className="builder-truncate-safe flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden rounded-md border border-[var(--app-border)] bg-[var(--app-panel-bg)] p-4 text-[var(--app-panel-text)] transition-colors duration-300" aria-label="File metadata">
      ...
      <dl className="mt-4 grid gap-2 text-[12px] leading-4">
        <div>
      ...
      {canPreviewText ? (
        <pre className="mt-sm min-h-0 min-w-0 flex-1 overflow-auto whitespace-pre-wrap break-words rounded-md border border-[var(--app-border)] bg-[var(--app-control)] p-2 font-mono text-[12px] leading-4 text-[var(--app-panel-text)] [overflow-wrap:anywhere]">
          {node.content}
        </pre>
      ) : node.type === 'file' ? (
        <p className="mt-4 rounded-md bg-[var(--app-control)] p-4 text-[12px] leading-4 text-[var(--app-muted-text)]">Text preview is not available for this file. Metadata is still safe to view.</p>
      ) : null}
    </section>
  )
}
