...
export function buildStoreSampleDataInstructions() {
  return [
    ...
    "When VITE_STORE_SLUG exists, StoreProvider MUST block rendering the app with a full-page loading skeleton while useStoreDetail is loading, and show a store load error UI with retry on error. Do not render sample fallback during this loading/error phase.",
    "Generated src/routes/__root.tsx MUST wrap <Outlet /> in React <Suspense fallback={<RouteSuspenseFallback />}> with a DESIGN.md-aligned skeleton fallback so client-side route/component suspension never shows a blank or broken UI. Keep RouteLoadingBar before SiteHeader and keep SiteFooter/Toaster outside the Suspense-wrapped Outlet.",
    ...
    "Generated route and component code that reads query/provider/API data MUST use optional chaining and nullish fallbacks for every data/entity read at every nested level before rendering or computing values. Examples: store?.name?.trim(), storeDetail?.setting?.currency ?? 'AUD', product?.descriptions?.trim() ?? '', product?.category?.name, product?.images?.[0], product?.models?.[0]?.name, product?.models?.length ?? 0. Treat optional chaining as the default style even when TypeScript marks a field as required; direct property access is allowed only after an explicit loading/error/missing-data guard proves the parent exists in that branch, and nullable children still use ?. / ??.",
    "Generated UI MUST normalize maybe-undefined API arrays before collection operations: (products ?? []).map(...), (product?.models ?? []).map(...), (product?.images ?? []), (order?.items ?? []).map(...). Never call .map, .filter, .reduce, .length, or [0] on possibly undefined API fields without optional chaining/nullish fallback. Product detail MUST return loading/error/missing-product UI before reading product fields.",
    "Any generated route or component that consumes useProductsList MUST handle productsQuery.isLoading with a product-grid skeleton, productsQuery.isError with a products load error UI and retry/refetch button, and isFetchingNextPage with a loading-more indicator. Do this in pages AND reusable components such as ProductGrid; never render an empty list while the query is pending.",
    "Any generated route or component that consumes useProductDetail MUST handle isLoading with a product-detail skeleton and isError or missing product data with a product load error UI and retry/refetch button before reading product fields. Do this in pages AND reusable product detail components.",
    ...
    "Generated src/routes/products/$productId.tsx MUST avoid render-loop state sync: never use useEffect to copy product/query data or derived arrays into state, and never call a setter from an effect depending on product, product.models, product.images, models, images, selectedModel, or freshly-created objects. Render: (1) image gallery — memoize images from product.images ?? (product.image ? [product.image] : []), useState<number>(0) for selectedImageIndex, derive safeImageIndex during render, and never reset the index from an effect; (2) model selector — memoize modelOptions from product.models/defaultModel, useState<string | null>(null) for selectedModelId (NOT selectedModel object state), derive selectedModel from modelOptions.find((m) => m.id === selectedModelId), do NOT preselect defaultModel/models[0], desktop-only chips wrapped in <div className='hidden md:block'> mapping modelOptions to selectable rounded-full <button> pills (active = selectedModelId === model.id with bg-primary text-primary-foreground); selecting a model re-renders the displayed price; (3) price — formatMoney(selectedModel?.price ?? product.defaultModel?.price ?? resolveProductPrice(product) ?? 0, { currency }); (4) descriptions read-more — use product.descriptions; the text is HTML and MUST be sanitized via DOMPurify.sanitize(product.descriptions ?? '') (`import DOMPurify from 'dompurify'` — `dompurify` is already a project dependency) and rendered through `dangerouslySetInnerHTML={{ __html: sanitized }}` on a wrapping <div> (NOT a <p>); memoize the sanitize call with useMemo keyed on product.descriptions; NEVER render product.descriptions raw without DOMPurify.sanitize; module-level const DESCRIPTION_THRESHOLD = 240; default line-clamp-4 on the wrapping <div> with a 'Read more' / 'Read less' toggle button when text length exceeds the threshold; (5) quantity stepper + Total row (formatMoney(selectedPrice * quantity, { currency })) + desktop Add-to-cart Button wrapped in a card container with className 'hidden space-y-4 rounded-2xl border bg-card p-5 md:block'; (6) mobile-only Add-to-cart Button (className 'fixed inset-x-4 bottom-4 z-40 md:hidden') that opens a <Sheet> from @/components/ui/sheet listing modelOptions as full-width selectable rows with formatMoney(model.price ?? 0, { currency }) right-aligned; the sheet header shows live price-per-model; the sheet also contains the quantity stepper, Total row and a Confirm Add-to-cart Button. The Sheet primitive lives at src/components/ui/sheet.tsx and is built on vaul Drawer.",
    "Product detail model/cart behavior MUST be explicit: initial selectedModelId is null and no default model is preselected; selecting a model sets selectedModelId and syncs quantity to getItemQuantity(model.id) when >0 or 1 otherwise; clicking Add/Update/Confirm with no selectedModel calls toast.error('Please choose a product option first') and does not mutate cart; the primary cart button must not be disabled solely because selectedModel is missing; valid add builds const payload = { product, model: selectedModel, quantity } and calls addItem(payload), valid update calls updateItemQuantity(selectedModel.id, quantity), quantity 0 removes existing items only, and every valid cart mutation calls toast.success(...).",
