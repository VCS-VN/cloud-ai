...
export function buildStoreRuntimeInstructions(input: {
  ...
  lines.push(
    "- Generated src/routes/__root.tsx MUST import Providers from @/app/providers and render exactly this provider order inside <body>: <Providers><StoreProvider><CartProvider><SiteHeader /><Outlet /><SiteFooter /><Toaster /></CartProvider></StoreProvider></Providers><Scripts />. NEVER remove Providers, NEVER place StoreProvider outside Providers, and NEVER call React Query hooks before QueryClientProvider is mounted. Generated vite.config.ts MUST render plugins in this exact order: plugins: [viteReact(), tanstackStart()]. React plugin must run before TanStack Start so Fast Refresh preamble is installed before transformed modules call $RefreshReg$.",
  );
  lines.push(
