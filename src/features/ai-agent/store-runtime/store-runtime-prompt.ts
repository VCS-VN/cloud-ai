...
export function buildStoreRuntimeInstructions(input: {
  ...
  lines.push(
    "- Generated src/routes/__root.tsx MUST import '@vitejs/plugin-react/preamble' first, then import '@/styles/app.css'. NEVER import '../app.css', './app.css', or any relative CSS path. It MUST import Providers from @/app/providers and render exactly this provider order inside <body>: <Providers><StoreProvider><CartProvider><RouteLoadingBar /><SiteHeader /><Suspense fallback={<RouteSuspenseFallback />}><Outlet /></Suspense><SiteFooter /><Toaster /></CartProvider></StoreProvider></Providers><Scripts />. RouteLoadingBar MUST render before SiteHeader. SiteFooter and Toaster MUST stay outside the Suspense-wrapped Outlet. RouteSuspenseFallback MUST be a DESIGN.md-aligned skeleton to prevent blank/broken client UI during route/component suspension. NEVER remove Providers, RouteLoadingBar, NEVER place StoreProvider outside Providers, and NEVER call React Query hooks before QueryClientProvider is mounted.",
  );
  lines.push(
