...
export function buildRetailInitPrompt(input: {
  ...
  return [
    ...
    "CREATE THESE FILES:",
    "",
    "PRE-SEEDED before the agent loop (already on disk — do NOT project_create_file; use write/edit only to customize): layout (theme-toggle, site-footer, route-loading-bar), not-found, all commerce routes (src/routes/index.tsx home, src/routes/products/*, cart, checkout, orders/*), providers, store hooks, and baseline store sections (hero, product-grid, etc.). You MUST still create src/routes/__root.tsx and src/components/layout/site-header.tsx. Do not remove commerce routes or their imports.",
    "",
    ...
    "",
    "src/components/layout/site-footer.tsx - PRE-SEEDED. Optional: patch with write/edit to match DESIGN.md (bg-deep text-deep-foreground, 4 cols Shop/Support/Company/Connect). __root.tsx already imports <SiteFooter />.",
    "",
    ...
    "",
    "src/components/store/hero-section.tsx - Editorial premium retail hero adapted to the store category and user prompt. Headline from websiteConfig.content.heroTitle must be large and memorable (max 2 lines desktop), with supporting copy (~20 words max), primary CTA Button to /products, optional secondary link/CTA, and a prominent product/category visual area. The visual MUST be a real photographic image: prefer a featured product image via product.image ?? product.images?.[0], otherwise a seeded Lorem Picsum lifestyle shot `https://picsum.photos/seed/<store-or-category-keyword>/1200/800` in an <img className='... object-cover' />. You MAY overlay small editorial detail cards (e.g. a stat or benefit chip) on the image using DESIGN.md token utilities. Do NOT use a decorative gradient-filled <div> as the hero visual, do NOT print the brand tone as an eyebrow, and do NOT create a generic SaaS landing hero, empty gray placeholder, AI-purple/mesh gradient, excessive animation, or raw visual literal outside DESIGN.md.",
    "",
    ...
    "",
    "src/components/store/product-grid.tsx - PRE-SEEDED. Title, grid-cols-1 sm:2 lg:3 xl:4 gap-6, map products to ProductCard. Use `const { storeDetail } = useStore()` and consume useProductsList({ storeId: storeDetail?.id }) — the hook uses useInfiniteQuery and returns { products, total, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading, isError, refetch }. The hook itself returns sample fallback data with hasNextPage=false when VITE_STORE_SLUG is missing. Render the products array via .map and append a sentinel <div ref={loadMoreRef} /> after the grid; use an IntersectionObserver effect that calls fetchNextPage() when the sentinel intersects the viewport AND hasNextPage AND !isFetchingNextPage (rootMargin: '200px'). Show 'Loading more...' inside the sentinel while isFetchingNextPage. Render a loading skeleton during isLoading and an error UI with retry on isError. The product count text MUST come from the hook's total field. Do NOT import { products } from @/data/products and do NOT branch on hasStoreSlug.",
    "",
    ...
    "",
    "COMMERCE ROUTES (PRE-SEEDED — patch with write/edit to match DESIGN.md; do NOT delete): src/routes/index.tsx (home), src/routes/products/route.tsx, src/routes/products/index.tsx, src/routes/products/$productId.tsx, src/routes/cart.tsx, src/routes/checkout.tsx, src/routes/orders/index.tsx, src/routes/orders/$orderId.tsx.",
    "",
    "Optional homepage polish: edit src/routes/index.tsx to add FeatureBand / NewsletterSection and a 5-section retail homepage per the taste skill — the baseline home route already includes HeroSection, CategorySection, ProductGrid, and TrustSignals.",
    "",
    "Optional: edit products routes for stronger catalog UX (search ?q, infinite scroll, product detail model picker) — baseline routes already implement core commerce behavior.",
    "",
    "src/routes/cart.tsx, checkout.tsx, orders/index.tsx, orders/$orderId.tsx — PRE-SEEDED; patch with write/edit for DESIGN.md polish only.",
    "",
