...
export function buildRetailInitPrompt(input: {
  ...
  return [
    ...
    "",
    "src/routes/__root.tsx - createRootRoute with component Root and notFoundComponent wired to NotFound, html/head(HeadContent)/body(Providers wrapping StoreProvider wrapping CartProvider wrapping SiteHeader/Outlet/SiteFooter/Toaster, then Scripts), import '@vitejs/plugin-react/preamble' first, then import '@/styles/app.css'. NEVER import '../app.css', './app.css', or any relative CSS path. NEVER remove Providers, notFoundComponent, or place StoreProvider outside Providers; StoreProvider uses React Query and MUST always be inside QueryClientProvider via Providers.",
    "",
