...
export function buildRetailInitPrompt(input: {
  ...
  return [
    ...
    "",
    "src/routes/__root.tsx - createRootRoute with component Root and notFoundComponent wired to NotFound, html/head(HeadContent)/body(Providers wrapping StoreProvider wrapping CartProvider wrapping RouteLoadingBar/SiteHeader/Suspense(Outlet)/SiteFooter/Toaster, then Scripts), import '@vitejs/plugin-react/preamble' first, then import '@/styles/app.css'. Wrap <Outlet /> in <Suspense fallback={<RouteSuspenseFallback />}> with a DESIGN.md-aligned skeleton fallback to prevent blank/broken client UI during route/component suspension. Keep SiteFooter and Toaster outside the Suspense-wrapped Outlet. NEVER import '../app.css', './app.css', or any relative CSS path. NEVER remove Providers, notFoundComponent, RouteLoadingBar, or place StoreProvider outside Providers; StoreProvider uses React Query and MUST always be inside QueryClientProvider via Providers.",
    "",
